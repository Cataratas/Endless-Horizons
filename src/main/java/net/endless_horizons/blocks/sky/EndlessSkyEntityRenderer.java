...
public class EndlessSkyEntityRenderer extends EndlessRenderer<EndlessSkyEntity> implements BlockEntityRenderer<EndlessSkyEntity> {
    ...
    public void render(EndlessSkyEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        //client.worldRenderer.renderSky(matrices, client.gameRenderer.getBasicProjectionMatrix(75), client.getTickDelta(), client.gameRenderer.getCamera(), false, () -> {});
        //RenderSystem.setShaderColor((float) colors.x, (float) colors.y, (float) colors.z, 1.0f);
        //RenderSystem.disableBlend();

        BufferBuilder buffer = Tessellator.getInstance().getBuffer();
        buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION);
        renderSides(entity, matrices.peek().getPositionMatrix(), buffer);
